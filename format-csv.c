...
char readCsv(EmbPattern* pattern, const char* fileName)
{
    ...
    if (!buff) { printf("ERROR: format-csv.c readCsv(), unable to allocate memory for buff\n"); return 0; }
    ...
    if (!file) return 0;
        ...
        do
        {
            ...
            if(pos >= size - 1)
            {
                ...
                if(!buff) { printf("ERROR: format-csv.c readCsv(), cannot re-allocate memory for buff\n"); return 0; }
            }
