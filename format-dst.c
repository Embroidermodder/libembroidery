...
static void encode_record(FILE* file, int x, int y, int flags)
{
    ...
    if(x > 121 || x < -121) printf("ERROR: format-dst.c encode_record(), x is not in valid range [-121,121] , x = %d\n", x);
    if(y > 121 || y < -121) printf("ERROR: format-dst.c encode_record(), y is not in valid range [-121,121] , y = %d\n", y);
