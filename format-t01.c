...
char readT01(EmbPattern* pattern, const char* fileName)
{
    ...
    if(!pattern) { printf("ERROR: format-t01.c readt01(), pattern argument is null\n"); return 0; }
    if(!fileName) { printf("ERROR: format-t01.c readt01(), fileName argument is null\n"); return 0; }
...
static void encode_record(EmbFile* file, int x, int y, int flags)
{
	...
	if (x > 121 || x < -121) printf("ERROR: format-t01.c encode_record(), x is not in valid range [-121,121] , x = %d\n", x);
	if (y > 121 || y < -121) printf("ERROR: format-t01.c encode_record(), y is not in valid range [-121,121] , y = %d\n", y);
	...
	if (x != 0) { printf("ERROR: format-dst.c encode_record(), x should be zero yet x = %d\n", x); }
	...
	if (y != 0) { printf("ERROR: format-dst.c encode_record(), y should be zero yet y = %d\n", y); }
