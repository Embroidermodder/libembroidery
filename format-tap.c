...
static void encode_record(FILE* file, int x, int y, int flags)
{
	...
	if (x > 121 || x < -121) printf("ERROR: format-tap.c encode_record(), x is not in valid range [-121,121] , x = %d\n", x);
	if (y > 121 || y < -121) printf("ERROR: format-tap.c encode_record(), y is not in valid range [-121,121] , y = %d\n", y);

	if (x >= +41) { b[2] += setbit(2); x -= 81; }
	if (x <= -41) { b[2] += setbit(3); x += 81; }
	if (x >= +14) { b[1] += setbit(2); x -= 27; }
	if (x <= -14) { b[1] += setbit(3); x += 27; }
	if (x >= +5) { b[0] += setbit(2); x -= 9; }
	if (x <= -5) { b[0] += setbit(3); x += 9; }
	if (x >= +2) { b[1] += setbit(0); x -= 3; }
	if (x <= -2) { b[1] += setbit(1); x += 3; }
	if (x >= +1) { b[0] += setbit(0); x -= 1; }
	if (x <= -1) { b[0] += setbit(1); x += 1; }
	if (x != 0) { printf("ERROR: format-tap.c encode_record(), x should be zero yet x = %d\n", x); }
	if (y >= +41) { b[2] += setbit(5); y -= 81; }
	if (y <= -41) { b[2] += setbit(4); y += 81; }
	if (y >= +14) { b[1] += setbit(5); y -= 27; }
	if (y <= -14) { b[1] += setbit(4); y += 27; }
	if (y >= +5) { b[0] += setbit(5); y -= 9; }
	if (y <= -5) { b[0] += setbit(4); y += 9; }
	if (y >= +2) { b[1] += setbit(7); y -= 3; }
	if (y <= -2) { b[1] += setbit(6); y += 3; }
	if (y >= +1) { b[0] += setbit(7); y -= 1; }
	if (y <= -1) { b[0] += setbit(6); y += 1; }
	if (y != 0) { printf("ERROR: format-tap.c encode_record(), y should be zero yet y = %d\n", y); }
