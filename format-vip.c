...
int readVip(EmbPattern* pattern, const char* fileName)
{
    ...
    if(!pattern) { printf("ERROR: format-vip.c readVip(), pattern argument is null\n"); return 0; }
    if(!fileName) { printf("ERROR: format-vip.c readVip(), fileName argument is null\n"); return 0; }
    ...
    /*stringVal = (unsigned char*)malloc(sizeof(unsigned char)*8); TODO: review this and uncomment or remove
        if(!stringVal) { printf("ERROR: format-vip.c readVip(), cannot allocate memory for stringVal\n"); return 0; }
     */
    ...
    if(!decodedColors) { printf("ERROR: format-vip.c readVip(), cannot allocate memory for decodedColors\n"); return 0; }
    ...
    if(!attributeData) { printf("ERROR: format-vip.c readVip(), cannot allocate memory for attributeData\n"); return 0; }
    ...
    if(!xData) { printf("ERROR: format-vip.c readVip(), cannot allocate memory for xData\n"); return 0; }
